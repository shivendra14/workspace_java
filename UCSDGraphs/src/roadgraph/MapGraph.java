...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		PriorityQueue<MapNode> toExplore = new PriorityQueue<MapNode>(getNumVertices(),queueComparator);
		HashSet<MapNode> visited = new HashSet<MapNode>();
		...
		int countPops=0;
			...
			double currDistance=next.getDistance();
			Set<MapNode> neighbors = getNeighbors(next);
			for (MapNode neighbor : neighbors) {
				if (!visited.contains(neighbor)) {
					MapEdge edge=next.getEdgeTo(neighbor);
					if ((currDistance + edge.getLength()) < neighbor.getDistance())
					{
						neighbor.setDistance(currDistance + edge.getLength());
						parentMap.put(neighbor, next);
						toExplore.add(neighbor);
					}
				}
		...
		System.out.println("Dijikstra pops"+ countPops);
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		int countPops=0;
			...
			double currDistance=next.getActualDistance();
			Set<MapNode> neighbors = getNeighbors(next);
			for (MapNode neighbor : neighbors) {
				if (!visited.contains(neighbor)) {
					MapEdge edge=next.getEdgeTo(neighbor);
					if ((currDistance + edge.getLength()) < neighbor.getActualDistance())
					{
						neighbor.setActualDistance(currDistance + edge.getLength());
						//use below one for quiz
		...
		System.out.println("AStar pops"+ countPops);
			return path;
	}
	...
	public static void main(String[] args)
	{
		...
		*/ 
		...
		route=theMap.dijkstra(new GeographicPoint(1.0,1.0), 
				 new GeographicPoint(8.0,-1.0));
		System.out.println("Dijkstra:\n"+route);
		
		route=theMap.aStarSearch(new GeographicPoint(1.0,1.0), 
				 new GeographicPoint(8.0,-1.0));
		...
		System.out.println("\n\nQUIZ:");
		System.out.println("No. of nodes are: "+route1.size());
		System.out.println("Dijkstra:\n"+route1);
		System.out.println("AStar:\n"+route2);		
	}
